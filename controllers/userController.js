...
const postUserData = async (req, res) => {
    try {
        const { userName, phoneNumber, email, location, date, room } = req.body;

        // Validate input data
        if (!userName || !phoneNumber || !email || !location || !date || !room) {
            return res.status(400).json({ Error: "All fields are required." });
        }

        // Create new user document
        const userData = new userDB({ userName, phoneNumber, email, location, date, room });
        console.log("User Data:", userData);
        await userData.save();

        // Send email
        await sendEmail(userName, phoneNumber, email, location, date, room);
...
const sendEmail = async (userName, phoneNumber, email, location, date, room) => {
    try {
        console.log("Sending email with data:", userName, phoneNumber, email, location, date, room);
        ...
        const mailOptions = {
            from: "Maa.greamsroad@gmail.com", // Corrected from `form` to `from`
            to: [email, "Maa.greamsroad@gmail.com"],
            subject: "Booking the room",
            text: `Dear ${userName},
                   Email: ${email}
